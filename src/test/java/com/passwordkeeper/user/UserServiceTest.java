...
public class UserServiceTest {
    ...
    public void should_return_existing_user_by_id() {
        when(userRepository.findById(1L)).thenReturn(Optional.of(new User(1L, "login", "124", 0, FREE.name())));
    ...
    public void should_update_user_account_type_by_id() {
        ...
        User userPassedToRepositoryWithPremiumAccountType = userArgumentCaptor.getValue();
        ...
        User userPassedToRepositoryWithFreeAccountType = userArgumentCaptor.getValue();
    ...
    public void should_find_user_by_login() {
        ...
        when(userRepository.findByLogin("login")).thenReturn(new User(1L, "login", "124", 0, FREE.name()));
    ...
    public void should_create_user_by_request() {
        ...
        when(userRepository.save(new User(null, "login", "124", 0, FREE.name())))
                .thenReturn(new User(1L, "login", "124", 0, FREE.name()));

        UserDto savedUser = userService.create(new UserRequestDto("login", "124"));

        verify(userRepository, times(1)).save(userArgumentCaptor.capture());

        User savingUser = userArgumentCaptor.getValue();
    ...
    public void should_increment_passwords_count() {
        when(userRepository.findById(1L)).thenReturn(Optional.of(new User(1L, "login", "124", 4, FREE.name())));
        boolean resultForFreeAccount = userService.passwordsCountIncremented("1");

        assertThat(resultForFreeAccount).isTrue();
        verify(userRepository, times(1)).save(userArgumentCaptor.capture());
        User savingValue = userArgumentCaptor.getValue();
        assertThat(savingValue.getPasswordsCount()).isEqualTo(5);

        when(userRepository.findById(2L)).thenReturn(Optional.of(new User(2L, "login", "124",9999 , PREMIUM.name())));
        boolean resultForPremiumAccount = userService.passwordsCountIncremented("2");

        assertThat(resultForPremiumAccount).isTrue();
        verify(userRepository, times(2)).save(userArgumentCaptor.capture());
        User savingValuePremium = userArgumentCaptor.getValue();
    ...
    public void should_not_increment_passwords_count() {
        ...
        when(userRepository.findById(1L)).thenReturn(Optional.of(new User(1L, "login", "124", 5, FREE.name())));
        boolean resultForFreeAccount = userService.passwordsCountIncremented("1");
        assertThat(resultForFreeAccount).isFalse();
        verify(userRepository, never()).save(any());

        when(userRepository.findById(2L)).thenReturn(Optional.of(new User(2L, "login", "124",10000 , PREMIUM.name())));
        boolean resultForPremiumAccount = userService.passwordsCountIncremented("2");
